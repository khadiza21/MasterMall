...
const contactData = [
    { icon: 'fa-solid fa-phone', lines: ['Toll-Free: 0000 - 000 - 0000', 'Fax: 0000 - 000 - 0000'] },
    { icon: 'fa-solid fa-envelope', lines: ['example@mail.com', 'support@mail.com'] },
    { icon: 'fa-solid fa-location-dot', lines: ['No: 58 A, East Madison Street,', 'Baltimore, MD, USA 4508'] }
  ];
  ...
  contactData.forEach(({ icon, lines }) => {
    ...
    box.innerHTML = `
      <div class="box">
        ...
        <div class="box-text">${lines.map(line => `<p>${line}</p>`).join('')}</div>
      </div>
    `;
